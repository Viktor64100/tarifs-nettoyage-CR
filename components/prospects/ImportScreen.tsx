...
export default function ImportScreen({ existingPhoneKeys }: { existingPhoneKeys: string[] }) {
  ...
  const existingSet = useMemo(() => new Set(existingPhoneKeys), [existingPhoneKeys]);
  const { imported, skippedCount, duplicateCount, usedHeader } = useMemo(
    () => mapRowsToProspects(parseCSV(raw), existingSet),
    [raw, existingSet]
  );
      ...
      {raw.trim() && (
        <div className="bg-card border border-line rounded-2xl p-4 mb-4">
          <div className="text-sm mb-2">
            ...
            {duplicateCount > 0 && (
              <span className="text-sub"> · {duplicateCount} doublon{duplicateCount > 1 ? "s" : ""} ignoré{duplicateCount > 1 ? "s" : ""} (déjà dans tes prospects)</span>
            )}
          </div>
